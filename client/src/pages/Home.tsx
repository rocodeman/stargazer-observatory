...
import { Camera, CheckCircle2, CircleUserRound, Compass, Crosshair, LocateFixed, Moon, Radio, RotateCcw, Stars, Sun, Telescope } from "lucide-react";
...
const MISSION_HINTS = {
  triad: { title: "提示 · 三个太阳", text: "三颗不属于此处的太阳，正在星海深处等待观测。点击任务面板，打开它们的运行记录。" },
  broadcast: { title: "提示 · 宇宙广播", text: "不要在黑夜呼喊。等白昼降临，寻找太阳，让信号继续传播。" },
  science: { title: "提示 · 科学家的葬礼", text: "四个永远沉默的科学家散落在观测视野中。他们不会主动呼救，请仔细检查整片天空。" },
  countdown: { title: "提示 · 倒计时", text: "观测记录需要被保存。让相机记住此刻，时间会留下证据。" },
  shine: { title: "提示 · 为你闪耀", text: "在夜幕下，让星座回应你的注视。切换至全部高亮，直到信号稳定。" },
} as const;
...
export default function Home() {
  ...
  const [missionHint, setMissionHint] = useState<{ title: string; text: string } | null>(null);
  ...
  const toggleAllConstellations = async () => {
    ...
    setSelectedConstellations((previous) => previous.size === data.constellations.length ? new Set(INITIAL_CONSTELLATION_IDS) : new Set(data.constellations.map((item) => item.id)));
  };
  ...
  return (
    <main className={`observatory ${dayMode ? "day-mode" : "night-mode"}`}>
      ...
      {allMissionsComplete && <section className="dark-forest-title" aria-label="第二部黑暗深林终局信息"><h2>现在，进入黑暗森林。你们只是尚未被发现的虫群。</h2><div className="dark-forest-events"><span>三个太阳的异常轨道已确认</span><span>宇宙广播已穿过半人马座 α</span><span>四位科学家的最后坐标已被发现</span><span>倒计时信号完成，观测者进入黑暗深林</span></div><strong className="dark-forest-ending">未完待续</strong><a href="https://github.com/rocodeman/stargazer-observatory" target="_blank" rel="noreferrer">GitHub · rocodeman/stargazer-observatory</a><a className="dark-forest-credit" href="https://github.com/changsheng0804-blip" target="_blank" rel="noreferrer">特别感谢 changsheng · GitHub</a></section>}
      ...
      <aside className="character-panel">
        <div className="character-profile"><div className="character-avatar"><CircleUserRound size={25} /></div><div><span>OBSERVATORY PILOT</span><strong>任务面板</strong></div></div>
        <div className="character-divider" />
        <div className="mission-panel-kicker"><Radio size={13} /> 任务 · 5</div>
        <div className={`mission-item ${triadComplete ? "complete" : "incomplete"}`} role="button" tabIndex={0} onClick={() => setShowVirtualSuns(true)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setShowVirtualSuns(true); }}>
          <span className="mission-status">{triadComplete ? <CheckCircle2 size={16} /> : <span className="mission-ring" />}</span><div><strong>三个太阳</strong><small>{triadComplete ? "已完成" : "未完成"}</small></div>
          <button type="button" className="mission-hint-button" onClick={(event) => { event.stopPropagation(); setMissionHint(MISSION_HINTS.triad); }} aria-label="查看三个太阳提示" title="查看提示">提示</button>
        </div>
        <div className={`mission-item ${broadcastComplete ? "complete" : "incomplete"}`}>
          <span className="mission-status">{broadcastComplete ? <CheckCircle2 size={16} /> : <span className="mission-ring" />}</span><div><strong>宇宙广播</strong><small>{broadcastComplete ? "已完成" : "未完成"}</small></div>
          <button type="button" className="mission-hint-button" onClick={() => setMissionHint(MISSION_HINTS.broadcast)} aria-label="查看宇宙广播提示" title="查看提示">提示</button>
        </div>
        <div className={`mission-item ${scienceBoundaryComplete ? "complete" : "incomplete"}`} role="button" tabIndex={0} onClick={() => { const missionTarget = scientistTargets.find((scientist) => scientist.missionOnly && !scientistsFound.has(scientist.id)); if (missionTarget) discoverScientist(missionTarget.id); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { const missionTarget = scientistTargets.find((scientist) => scientist.missionOnly && !scientistsFound.has(scientist.id)); if (missionTarget) discoverScientist(missionTarget.id); } }}>
          <span className="mission-status">{scienceBoundaryComplete ? <CheckCircle2 size={16} /> : <span className="mission-ring" />}</span><div><strong>科学家的葬礼</strong><small>{scienceBoundaryComplete ? "已完成" : `还需发现 ${scientistTargets.length - scientistsFound.size} 位完成`}</small></div>
          <button type="button" className="mission-hint-button" onClick={(event) => { event.stopPropagation(); setMissionHint(MISSION_HINTS.science); }} aria-label="查看科学家的葬礼提示" title="查看提示">提示</button>
        </div>
        <div className={`mission-item ${countdownComplete ? "complete" : "incomplete"}`}>
          <span className="mission-status">{countdownComplete ? <CheckCircle2 size={16} /> : <span className="mission-ring" />}</span><div><strong>倒计时</strong><small>{countdownComplete ? "已完成" : "未完成"}</small></div>
          <button type="button" className="mission-hint-button" onClick={() => setMissionHint(MISSION_HINTS.countdown)} aria-label="查看倒计时提示" title="查看提示">提示</button>
        </div>
        <div className={`mission-item ${shineComplete ? "complete" : "incomplete"}`}>
          <span className="mission-status">{shineComplete ? <CheckCircle2 size={16} /> : <span className="mission-ring" />}</span><div><strong>为你闪耀</strong><small>{shineComplete ? "已完成" : "未完成"}</small></div>
          <button type="button" className="mission-hint-button" onClick={() => setMissionHint(MISSION_HINTS.shine)} aria-label="查看为你闪耀提示" title="查看提示">提示</button>
        </div>
        <button type="button" className="mission-replay" onClick={replayMissions}><RotateCcw size={12} /> 重玩</button>
      </aside>
      {missionHint && <button type="button" className="mission-hint-overlay" onClick={() => setMissionHint(null)} aria-label="关闭任务提示"><span className="mission-hint-card"><strong>{missionHint.title}</strong><p>{missionHint.text}</p><small>点击文字关闭提示</small></span></button>}
      <header className="topbar"><div className="brand"><button className="brand-camera-button" onClick={captureObservatory} aria-label="拍摄天文台截图" title="拍摄天文台截图"><Camera size={18} /></button><div><div className="eyebrow">NIGHT OBSERVATORY · 3D SKY</div><h1>三体</h1></div></div><div className="status"><span className="status-dot" /> HYG v4.1 · {displayStarCount.toLocaleString()} STARS <b>·</b> {displayConstellationCount} CONSTELLATIONS</div><div className="topbar-controls"><button className="icon-button" onClick={toggleDayNight} aria-label={dayMode ? "切换到夜间观测" : "切换到白天观测"} title={dayMode ? "切换到夜间观测" : "切换到白天观测"}>{dayMode ? <Moon size={17} /> : <Sun size={17} />}</button><button className={`icon-button constellation-icon-button ${allConstellationsSelected ? "active" : ""}`} onClick={() => void toggleAllConstellations()} aria-label={allConstellationsSelected ? "恢复首屏 20 个星座" : "显示全部 89 个星座"} title={allConstellationsSelected ? "恢复首屏 20 个星座" : "显示全部 89 个星座"}><Stars size={17} /></button></div></header>
