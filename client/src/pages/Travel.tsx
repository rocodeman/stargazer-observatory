...
export default function Travel() {
  ...
  return (
    <main className={`travel-page ${arrived ? "arrival-mode" : "cruise-mode"}`}>
      ...
      {!arrived ? <>
        <aside className="flight-console"><div className="mission-target"><div className="readout-label">DESTINATION LOCK</div><h2>{planet.name}</h2><span>{planet.latin} · {planet.orbit}</span></div><p className="mission-copy">{planet.description}</p></aside>
        <div className="flight-status"><Gauge size={14} /> {paused ? "航行状态 · 已暂停" : "航行状态 · 航行中"}</div><div className="bottom-telemetry"><div className="bottom-telemetry-row"><span><small>PROGRESS</small>{progress.toFixed(1)}%</span><span><small>RANGE</small>{planet.distance}</span><span><small>ETA</small>{planet.travel}</span><span><small>EARTH TIME</small>{earthTime}</span></div></div><div className="center-flight-controls"><button className="cockpit-control" aria-pressed={paused} onClick={() => setPaused(!paused)}><span className="control-led" />{paused ? <Play size={15} /> : <Gauge size={15} />}<span><strong>{paused ? "推进恢复" : "推力保持"}</strong><small>{paused ? "THRUST RESUME" : "THRUST HOLD"}</small></span></button><button className="cockpit-control lock-control" onClick={() => { setProgress(100); setArrived(true); setPaused(false); }}><span className="control-led" /> <Rocket size={15} /><span><strong>航线锁定</strong><small>TARGET LOCK · APPROACH</small></span></button></div><div className="ai-log-panel"><div className="panel-kicker"><span className="panel-live" /> AI NAV LOG · LIVE</div><div className="log-time">T+ {progress.toFixed(1)}% · STAGE {String(logStage + 1).padStart(2, "0")}</div><p>{aiLogs[logStage]}</p><div className="log-stream">{aiLogs.slice(0, logStage + 1).map((entry, index) => <span key={entry} className={index === logStage ? "current" : ""}>› {entry}</span>)}</div></div><div className="navigation-panel planet-list-panel"><div className="panel-kicker">PLANETS · 6 TARGETS</div><div className="planet-list">{Object.entries(PLANETS).map(([id, item]) => <button key={id} className={targetId === id ? "planet-list-item active" : "planet-list-item"} onClick={() => changeTarget(id)}><span className="planet-list-dot" style={{ background: item.color }} /><span className="planet-list-copy"><strong>{item.name}</strong><small>{item.latin}</small></span></button>)}</div></div>
